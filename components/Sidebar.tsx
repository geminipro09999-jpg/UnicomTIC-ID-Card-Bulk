...
// Import Layers icon which was missing and causing a compilation error
import { 
...
interface SidebarProps {
  ...
  setSettings: any; // Simplified to allow functional updates
  editScope: 'global' | 'individual';
  setEditScope: (scope: 'global' | 'individual') => void;
  clearOverrides: () => void;
...
const Sidebar: React.FC<SidebarProps> = ({
  settings, setSettings, editScope, setEditScope, clearOverrides, customLogo, setCustomLogo, 
  ...
  const resetDimensions = () => setSettings((p: any) => ({ ...p, cardWidthMM: 85.6, cardHeightMM: 54 }));
  ...
  return (
    <div className="w-full md:w-96 bg-white border-r border-gray-200 p-4 flex flex-col gap-2 no-print overflow-y-auto h-screen sticky top-0 shadow-xl z-20">
      ...
      {/* Scope Toggler */}
      <div className="bg-teal-50 p-3 rounded-xl border border-teal-100 mb-2">
        <div className="flex justify-between items-center mb-2">
            <span className="text-[10px] font-bold text-teal-900 uppercase">Design Scope</span>
            {editScope === 'individual' && selectedParticipant?.designOverrides && (
                <button onClick={clearOverrides} className="text-[10px] text-red-600 hover:underline font-bold flex items-center gap-1"><RefreshCw size={10}/> Reset Card</button>
            )}
        </div>
        <div className="flex bg-white p-1 rounded-lg shadow-sm border border-teal-200">
            <button 
                onClick={() => setEditScope('global')}
                className={`flex-1 py-2 text-xs font-bold rounded-md transition-all flex items-center justify-center gap-2 ${editScope === 'global' ? 'bg-teal-600 text-white shadow-md' : 'text-gray-400 hover:text-gray-600'}`}
            >
                <Layers size={14}/> Bulk Edit
            </button>
            <button 
                onClick={() => setEditScope('individual')}
                className={`flex-1 py-2 text-xs font-bold rounded-md transition-all flex items-center justify-center gap-2 ${editScope === 'individual' ? 'bg-teal-600 text-white shadow-md' : 'text-gray-400 hover:text-gray-600'}`}
            >
                <Paintbrush size={14}/> This Card
            </button>
        </div>
        {editScope === 'individual' && !selectedParticipant && (
            <p className="text-[10px] text-red-500 mt-2 font-bold italic text-center animate-pulse">Select a participant below to adjust this specific card.</p>
        )}
      </div>
            ...
            <div className="max-height-[300px] overflow-y-auto divide-y divide-gray-100 bg-white" style={{ maxHeight: '300px' }}>
              {filteredParticipants.length > 0 ? (
                filteredParticipants.map((p) => {
                  ...
                  const hasOverrides = !!p.designOverrides;
                  return (
                    <div 
                        key={originalIdx} 
                    ...
                    >
                      <div className="min-w-0 flex-grow">
                        <div className="flex items-center gap-2">
                            ...
                            {hasOverrides && <span className="text-[8px] bg-orange-100 text-orange-700 px-1 rounded font-bold uppercase">Custom</span>}
                        </div>
      ...
      {openSections.data && (
        <div className="p-2 space-y-4 mb-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div><label className="text-[10px] font-bold text-gray-600">Default Date</label><input type="text" value={settings.globalDate} onChange={(e) => setSettings((p:any) => ({ ...p, globalDate: e.target.value }))} className="w-full mt-1 p-1.5 border rounded text-xs" /></div>
            <div><label className="text-[10px] font-bold text-gray-600">Start ID</label><input type="number" value={settings.startId} onChange={(e) => setSettings((p:any) => ({ ...p, startId: e.target.value }))} className="w-full mt-1 p-1.5 border rounded text-xs" /></div>
          </div>
        </div>
      )}
      ...
      {openSections.layout && (
        <div className="p-2 space-y-4 mb-4">
          <div className="grid grid-cols-2 gap-2">{['A4', 'A3', 'A2', 'CR80'].map(size => (<button key={size} type="button" onClick={() => setSettings((p:any) => ({ ...p, pageSize: size }))} className={`py-2 px-3 rounded text-sm font-medium transition-colors ${settings.pageSize === size ? 'bg-teal-600 text-white shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>{size === 'CR80' ? 'Single' : size}</button>))}</div>
          {settings.pageSize !== 'CR80' && (
            <div className="bg-gray-50 p-3 rounded-lg border border-gray-200 space-y-3">
              <div className="flex justify-between items-center"><span className="text-xs font-semibold text-gray-700">Grid</span><div className="flex gap-2 text-xs"><button type="button" onClick={() => setSettings((p:any) => ({ ...p, manualGrid: { ...p.manualGrid, enabled: false } }))} className={`px-2 py-1 rounded ${!settings.manualGrid.enabled ? 'bg-teal-600 text-white' : 'bg-gray-200 text-gray-600'}`}>Auto</button><button type="button" onClick={() => setSettings((p:any) => ({ ...p, manualGrid: { ...p.manualGrid, enabled: true } }))} className={`px-2 py-1 rounded ${settings.manualGrid.enabled ? 'bg-teal-600 text-white' : 'bg-gray-200 text-gray-600'}`}>Manual</button></div></div>
              {settings.manualGrid.enabled ? (
                <div className="grid grid-cols-2 gap-3">
                  <div><label className="text-[10px] text-gray-500 uppercase font-bold">Cols</label><input type="number" value={settings.manualGrid.cols} onChange={(e) => setSettings((p:any) => ({ ...p, manualGrid: { ...p.manualGrid, cols: Math.max(1, parseInt(e.target.value) || 1) } }))} className="w-full mt-1 p-1.5 border rounded text-xs text-center" /></div>
                  <div><label className="text-[10px] text-gray-500 uppercase font-bold">Rows</label><input type="number" value={settings.manualGrid.rows} onChange={(e) => setSettings((p:any) => ({ ...p, manualGrid: { ...p.manualGrid, rows: Math.max(1, parseInt(e.target.value) || 1) } }))} className="w-full mt-1 p-1.5 border rounded text-xs text-center" /></div>
                </div>
              ) : <div className="text-xs text-gray-500 italic text-center">Auto-fitting <b>{layoutConfig.cols}</b>×<b>{layoutConfig.rows}</b>.</div>}
            </div>
          )}
          <div className="pt-2"><span className="text-xs font-bold text-gray-700 block mb-2"><Scissors size={12} className="inline mr-1" /> Cutting Marks</span><div className="flex bg-gray-100 p-1 rounded-lg">{(['none', 'border', 'crop'] as const).map(type => (<button key={type} type="button" onClick={() => setSettings((p:any) => ({ ...p, cutMarkType: type }))} className={`flex-1 py-1.5 text-xs font-medium rounded transition-all capitalize ${settings.cutMarkType === type ? 'bg-white shadow text-teal-700' : 'text-gray-500 hover:text-gray-700'}`}>{type}</button>))}</div></div>
        </div>
      )}
      ...
      {openSections.design && (
        <div className="p-2 space-y-4 mb-4">
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label className="text-[10px] text-gray-500 uppercase font-bold">W</label><input type="number" step="0.1" value={toDisplayUnit(settings.cardWidthMM)} onChange={(e) => setSettings((p:any) => ({ ...p, cardWidthMM: fromDisplayUnit(e.target.value) }))} className="w-full mt-1 p-1.5 border rounded text-xs" /></div>
              <div><label className="text-[10px] text-gray-500 uppercase font-bold">H</label><input type="number" step="0.1" value={toDisplayUnit(settings.cardHeightMM)} onChange={(e) => setSettings((p:any) => ({ ...p, cardHeightMM: fromDisplayUnit(e.target.value) }))} className="w-full mt-1 p-1.5 border rounded text-xs" /></div>
            </div>
          ...
          <div className="bg-gray-50 p-2 rounded-lg border border-gray-100">
            ...
            <div className="mt-2 mb-4"><AlignmentPicker horizontal={settings.logoAlign.horizontal} vertical={settings.logoAlign.vertical} onHChange={(h) => setSettings((p:any) => ({ ...p, logoAlign: { ...p.logoAlign, horizontal: h } }))} onVChange={(v) => setSettings((p:any) => ({ ...p, logoAlign: { ...p.logoAlign, vertical: v } }))} /></div>
            <span className="text-[10px] font-bold text-gray-700 uppercase">Logo Offsets</span>
            <div className="space-y-3 mt-2">
              <div><div className="flex justify-between text-[10px] mb-1"><span>Size</span><span>{settings.logoSize}px</span></div><input type="range" min="30" max="250" value={settings.logoSize} onChange={(e) => setSettings((p:any) => ({ ...p, logoSize: Number(e.target.value) }))} className="w-full h-1 bg-gray-200 rounded accent-teal-600" /></div>
              <div className="grid grid-cols-2 gap-2">
                <div><span className="text-[10px]">X Adjust</span><input type="range" min="-100" max="200" value={settings.logoPos.x} onChange={(e) => setSettings((p:any) => ({ ...p, logoPos: { ...p.logoPos, x: Number(e.target.value) } }))} className="w-full h-1 bg-gray-200 rounded accent-teal-600" /></div>
                <div><span className="text-[10px]">Y Adjust</span><input type="range" min="-100" max="100" value={settings.logoPos.y} onChange={(e) => setSettings((p:any) => ({ ...p, logoPos: { ...p.logoPos, y: Number(e.target.value) } }))} className="w-full h-1 bg-gray-200 rounded accent-teal-600" /></div>
              </div>
      ...
      {openSections.typo && (
        <div className="p-2 space-y-4 mb-4">
          <div className="bg-gray-50 p-2 rounded-lg border border-gray-100">
            ...
            <div className="mb-3"><AlignmentPicker horizontal={settings.nameAlign.horizontal} vertical={settings.nameAlign.vertical} onHChange={(h) => setSettings((p:any) => ({ ...p, nameAlign: { ...p.nameAlign, horizontal: h } }))} onVChange={(v) => setSettings((p:any) => ({ ...p, nameAlign: { ...p.nameAlign, vertical: v } }))} /></div>
            <div className="space-y-3">
              <div>
                <label className="text-[10px] font-bold text-gray-500 uppercase block mb-1">Font Family</label>
                <select className="w-full p-2 text-xs border rounded bg-white shadow-sm" value={settings.fontFamilies.name} onChange={e => setSettings((p:any) => ({...p, fontFamilies: {...p.fontFamilies, name: e.target.value}}))}>
                    {FONTS.map(f => <option key={f} value={f} style={{fontFamily: f}}>{f}</option>)}
                </select>
              </div>
              <div><span className="text-[10px] font-bold text-gray-500 uppercase">Size {settings.fontSizes.name}pt</span><input type="range" min="8" max="60" value={settings.fontSizes.name} onChange={(e) => setSettings((p:any) => ({ ...p, fontSizes: { ...p.fontSizes, name: Number(e.target.value) } }))} className="w-full h-1 bg-gray-200 accent-teal-600" /></div>
              <div className="grid grid-cols-2 gap-2">
                <div><span className="text-[10px]">X Adj</span><input type="range" min="-200" max="200" value={settings.namePos.x} onChange={(e) => setSettings((p:any) => ({ ...p, namePos: { ...p.namePos, x: Number(e.target.value) } }))} className="w-full h-1 bg-gray-200 accent-teal-600" /></div>
                <div><span className="text-[10px]">Y Adj</span><input type="range" min="-100" max="100" value={settings.namePos.y} onChange={(e) => setSettings((p:any) => ({ ...p, namePos: { ...p.namePos, y: Number(e.target.value) } }))} className="w-full h-1 bg-gray-200 accent-teal-600" /></div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gray-50 p-2 rounded-lg border border-gray-100">
            ...
            <div className="mb-3"><AlignmentPicker horizontal={settings.idAlign.horizontal} vertical={settings.idAlign.vertical} onHChange={(h) => setSettings((p:any) => ({ ...p, idAlign: { ...p.idAlign, horizontal: h } }))} onVChange={(v) => setSettings((p:any) => ({ ...p, idAlign: { ...p.idAlign, vertical: v } }))} /></div>
            <div className="space-y-3">
              <div>
                <label className="text-[10px] font-bold text-gray-500 uppercase block mb-1">Font Family</label>
                <select className="w-full p-2 text-xs border rounded bg-white shadow-sm" value={settings.fontFamilies.id} onChange={e => setSettings((p:any) => ({...p, fontFamilies: {...p.fontFamilies, id: e.target.value}}))}>
                    {FONTS.map(f => <option key={f} value={f} style={{fontFamily: f}}>{f}</option>)}
                </select>
              </div>
              <div><span className="text-[10px] font-bold text-gray-500 uppercase">Size {settings.fontSizes.id}pt</span><input type="range" min="10" max="60" value={settings.fontSizes.id} onChange={(e) => setSettings((p:any) => ({ ...p, fontSizes: { ...p.fontSizes, id: Number(e.target.value) } }))} className="w-full h-1 bg-gray-200 accent-teal-600" /></div>
              <div className="grid grid-cols-2 gap-2">
                <div><span className="text-[10px]">X Adj</span><input type="range" min="-200" max="200" value={settings.idPos.x} onChange={(e) => setSettings((p:any) => ({ ...p, idPos: { ...p.idPos, x: Number(e.target.value) } }))} className="w-full h-1 bg-gray-200 accent-teal-600" /></div>
                <div><span className="text-[10px]">Y Adj</span><input type="range" min="-100" max="100" value={settings.idPos.y} onChange={(e) => setSettings((p:any) => ({ ...p, idPos: { ...p.idPos, y: Number(e.target.value) } }))} className="w-full h-1 bg-gray-200 accent-teal-600" /></div>
              </div>
            </div>
          </div>
        </div>
